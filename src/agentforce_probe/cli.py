...
def cmd_doctor(args):
    ...
    _print("")
    for c in checks:
        _print("%s %-22s %s" % (icons.get(c["status"], "?"), c["name"], c["detail"]))
    _print("")
...
def cmd_run(args):
    ...
    if getattr(args, "from_results", None):
        ...
        start = raw.find("{")
        if start < 0:
            _err("no JSON object found in %s" % args.from_results)
            return 2
        ...
        _print("Agent: %s  ·  offline re-score from %s" % (agent_name, args.from_results))
        _print("")
        _print("Score: %d/%d = %d%%  ·  topic %d/%d · actions %d/%d · output %d/%d" % (
            total_pass, total, pct,
            agg["topic"][0], agg["topic"][1],
            agg["actions"][0], agg["actions"][1],
            agg["output"][0], agg["output"][1]))
        out_path = args.out or ("%s-evidence.md" % agent_name)
        content = evidence_mod.render_evidence(
            agent_name=agent_name, org_alias=args.org,
            agent_type="ExternalCopilot (offline re-score)",
            path_label="Offline re-score of existing `sf agent test` results",
            results=scored, judge_label="Salesforce Testing Center (recorded)")
        evidence_mod.write_evidence(out_path, content)
        _print("Evidence written: %s" % out_path)
        return 0
    ...
    if args.dry_run and args.force_type:
        ...
        meta = {"id": args.bot_id, "type": args.force_type, "is_internal": is_internal,
                "label": agent_name, "developer_name": agent_name}
    else:
        from . import agent_meta
        try:
            meta = agent_meta.resolve_agent(args.org, agent_name)
        except Exception as e:
            _err("could not resolve agent metadata: %s" % e)
            return 3
        is_internal = meta["is_internal"]

    _print("Agent: %s  ·  type: %s  ·  path: %s" % (
        agent_name, meta.get("type"),
    ...
    out_path = args.out or ("%s-evidence.md" % agent_name)
    content = evidence_mod.render_evidence(
        agent_name=agent_name, org_alias=args.org, agent_type=meta.get("type"),
        path_label=path_label, results=results, judge_label=judge_label)
    evidence_mod.write_evidence(out_path, content)
    _print("Evidence written: %s" % out_path)
    return 0
...
def _run_internal(args, cfg, spec, meta, judge_label, agent_name):
    ...
    if provider == "handoff":
        raw = sf_internal.run_session(
            spec=spec, instance_url=instance_url, bot_definition_id=bot_id,
            consumer_key=ck, consumer_secret=cs,
            diag=lambda m: _print("  [diag] %s" % m))
    ...
    if provider != "mock":
        ...
        if not judge_api_key:
            raise RuntimeError("no API key for judge provider '%s'" % provider)

    return sf_internal.run_internal(
        spec=spec, instance_url=instance_url, bot_definition_id=bot_id,
        consumer_key=ck, consumer_secret=cs,
        judge_provider=provider, judge_model=model, judge_api_key=judge_api_key,
        diag=lambda m: _print("  [diag] %s" % m))
...
def _emit_handoff(args, spec, agent_name, raw_results):
    ...
    judge_mod.write_judging_md(
        judging_path, judge_mod.render_judging_md(agent_name, task_path, verdicts_p))

    n = len(task["cases"])
    _print("")
    _print("Judge mode: handoff (Claude Code) — NO LLM was contacted.")
    _print("Replayed %d case(s) through the Agent API; wrote the judging materials:" % n)
    _print("  • task package : %s" % task_path)
    _print("  • instructions : %s" % judging_path)
    _print("")
    _print("Next steps:")
    _print("  1. Open Claude Code in this repo and paste the block from:")
    _print("       %s" % judging_path)
    ...
    out_flag = (" --out %s" % args.out) if args.out else ""
    _print("       python3 -m agentforce_probe run --org %s --agent %s --spec %s \\"
           % (args.org, agent_name, args.spec))
    _print("         --from-verdicts %s%s" % (verdicts_p, out_flag))
    _print("")
...
def _run_from_verdicts(args, spec, agent_name, scorer, evidence_mod):
    ...
    if not task_path:
        ...
        if not _os.path.exists(task_path):
            ...
            sib = _os.path.join(
                _os.path.dirname(_os.path.abspath(args.from_verdicts)),
                "%s-judge-task.json" % agent_name)
    ...
    try:
        ...
        _err("handoff error: %s" % e)
        return 2

    if task.get("schema") != judge_mod.TASK_SCHEMA:
        _err("WARN: task package schema is %r, expected %r"
             % (task.get("schema"), judge_mod.TASK_SCHEMA))
    for w in warnings:
        _err("WARN: %s" % w)
    ...
    if missing:
        _err("verdicts are missing case id(s): %s"
    ...
    if extra:
        _err("WARN: verdicts contain unknown id(s) not in the task package: %s"
    ...
    for tc in task_cases:
        cid = int(tc["id"])
        spec_case = spec_cases[cid - 1] if 1 <= cid <= len(spec_cases) else {
        ...
        scored.append(scorer.score_case(
            spec_case, cid, topic_pass=topic_pass, actions_pass=actions_pass,
            output_pass=output_pass, response=tc.get("actual_response", ""),
            actual_topic=actual_topic, actual_actions=actual_actions,
            judge_reason=v["reason"]))

    agg, total_pass, total = scorer.aggregate(scored)
    pct = round(100 * total_pass / total) if total else 0
    _print("Agent: %s  ·  collecting Claude Code verdicts from %s"
           % (agent_name, args.from_verdicts))
    _print("")
    _print("Score: %d/%d = %d%%  ·  topic %d/%d · actions %d/%d · output %d/%d" % (
        total_pass, total, pct,
        agg["topic"][0], agg["topic"][1],
    ...
    content = evidence_mod.render_evidence(
        ...
        results=scored, judge_label="Claude Code (file handoff)")
    evidence_mod.write_evidence(out_path, content)
    _print("Evidence written: %s" % out_path)
    return 0
...
def build_parser():
    ...
    pr.add_argument("--judge", help="judge for the Internal path. `handoff` "
    ...
    pr.add_argument("--from-results", dest="from_results",
    ...
    pr.add_argument("--from-verdicts", dest="from_verdicts",
                    help="offline (handoff step ③): collect Claude Code verdicts. "
                         ...
                         "No org/LLM call.")
    pr.add_argument("--judge-task", dest="judge_task",
    ...
    pr.add_argument("--dry-run", action="store_true",
                    help="local validation only; refuses to call org/Einstein")
    pr.add_argument("--force-type", choices=["internal", "external"],
                    help="(dry-run aid) skip org lookup and assume this type")
