...
def run_session(*, spec, instance_url, bot_definition_id, consumer_key,
                consumer_secret, diag=None, session_factory=None):
    ...
    if session_factory is not None:
        session = session_factory(
            instance_url, bot_definition_id, consumer_key, consumer_secret, _diag)
    else:
        minted = agent_api.mint_token(instance_url, consumer_key, consumer_secret)
        shape = minted["shape"]
        _diag("minted token: segments=%d len=%d (JWT ok)" % (shape["segments"], shape["len"]))
        _diag("agent api host: %s" % minted["api_instance_url"])
        session = agent_api.AgentApiSession(
            minted["api_instance_url"], minted["token"], bot_definition_id)

    sid = session.start()
    _diag("session started: %s" % sid)
    ...
    try:
        for i, c in enumerate(spec["testCases"], start=1):
            ...
            _diag("case %d replayed (response len=%d)" % (i, len(raw[-1]["response"])))
    finally:
        session.close()
...
def score_session(*, spec, raw_results, judge_fn, diag=None):
    ...
    for raw in raw_results:
        i = raw["number"]
        c = cases[i - 1]
        topic_pass, actions_pass = _topic_actions_for_case(
            c, raw.get("topic"), raw.get("actions"))
        passed, reason = judge_fn(
            c.get("expectedOutcome"), raw.get("response"), utterance=raw.get("utterance"))
        scored.append(score_case(
            c, i, topic_pass=topic_pass, actions_pass=actions_pass,
            output_pass=passed, response=raw.get("response", ""),
            actual_topic=raw.get("topic"), actual_actions=raw.get("actions"),
            judge_reason=reason))
        _diag("case %d scored (output=%s)" % (i, "PASS" if passed else "FAIL"))
    return scored


def run_internal(*, spec, instance_url, bot_definition_id, consumer_key,
                 consumer_secret, judge_provider, judge_model, judge_api_key,
                 diag=None):
    ...
    """
    raw = run_session(
        spec=spec, instance_url=instance_url, bot_definition_id=bot_definition_id,
        consumer_key=consumer_key, consumer_secret=consumer_secret, diag=diag)
    ...
    def _judge(expected_outcome, actual_response, utterance=None):
        return judge_mod.judge_case(
            ...
            utterance, expected_outcome, actual_response)
