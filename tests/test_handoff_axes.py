...
def test_load_verdicts_v2_high_axes_pass(tmp_path):
    obj = {"schema": judge_mod.VERDICTS_SCHEMA, "agent": "A", "verdicts": [_axis_entry(1, 0.9, "good")]}
...
def test_load_verdicts_v2_low_axes_fail(tmp_path):
    obj = {"schema": judge_mod.VERDICTS_SCHEMA, "agent": "A", "verdicts": [_axis_entry(1, 0.15, "bad")]}
...
def test_load_verdicts_v1_legacy_back_compat(tmp_path):
    obj = {
        "schema": judge_mod.VERDICTS_SCHEMA_V1,
        "agent": "A",
...
def test_load_verdicts_rejects_empty_entry(tmp_path):
    obj = {"schema": judge_mod.VERDICTS_SCHEMA, "agent": "A", "verdicts": [{"id": 1, "reason": "no verdict, no axes"}]}
...
def test_evidence_renders_axes_when_present():
    ...
    md = evidence.render_evidence(
        ...
        path_label="p",
        results=[_result_with_axes(axes)],
        judge_label="anthropic",
    )
    assert "axes:" in md
...
def test_evidence_no_axis_block_when_axes_none():
    md = evidence.render_evidence(
        ...
        agent_type="InternalCopilot",
        path_label="p",
        results=[_result_with_axes(None)],
        judge_label="anthropic",
    )
